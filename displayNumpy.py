...
USAGE:
=============
from displayNumpy import *
loadDisplay = DisplayNumpy()  
...
class DisplayNumpy(object):
    ...
    def dicomTransform(self, image, image_pos_pat, image_ori_pat):
        ...
        IP =  np.array([0, 0, 0]) # Initialization Image Position
        ...
        reslice.SetResliceAxes( DICOM_mat )
        reslice.SetInput( image )
        reslice.Update()
          
        # Change info origin
        origin_image = vtk.vtkImageChangeInformation()
        origin_image.SetInput( reslice.GetOutput() );
        origin_image.SetOutputOrigin(origin[0,0], origin[0,1], origin[0,2])
        origin_image.Update()
        
        return origin_image.GetOutput(), transform   
