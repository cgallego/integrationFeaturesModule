...
class convertNumpy(object):
    ...
    def createMeshfromMask(self, npmask, t_ImagedataVTK, npImagesandMask):
        """ Takes a binary mask image and makes it a vtkPolyData VOI_mesh """                
           
        vtkimMask = self.convertArray2vtkImage(npmask, t_ImagedataVTK, npImagesandMask)
    ...
    def convertDCEArray2vtkImage(self, npImagesandMask, postS):        
        #####################
        # Create vtk object
        size_array = npImagesandMask['dims'][2]*npImagesandMask['dims'][1]*npImagesandMask['dims'][0]
        flatim = npImagesandMask['im'+str(postS)]
        flatim = flatim.transpose(2,1,0).flatten()
  
        # create vtk image
        vtk_image = vtk.vtkImageData()
        vtk_image.SetDimensions(npImagesandMask['dims'])
        vtk_image.SetOrigin(0,0,0)
        vtk_image.SetSpacing(npImagesandMask['spacing'])
        vtk_image.SetNumberOfScalarComponents(1)
        vtk_image.SetScalarTypeToDouble()
        vtk_image.AllocateScalars()
    ...
    def testVTK2pOutputfile(self):
        ...
        # Open filename list
        StudyID = '714'    
        DicomExamNumber = '12545' # corresponds to old way of ret
        Lesions_id = '1293'
        SeriesID = 'S800' # corresponds to dynamic sequence;
        ...
        print "\n Visualize volumes..."
        loadDisplay = Display()
        lesion3D_mesh = loadDisplay.addSegment(lesion3D, [1,0,0], False)
        ...
        print "\n Converting DICOM data to list of arrays [x,y,z] and lesion3D segmentation to mask [x,y,z].... (only required for numpy testing)"
        self.npDICOMImages = {}
